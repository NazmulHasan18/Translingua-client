  ...
  const handelDeleteClass = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const res = await apiInstance.delete(`/students/selected/${id}?email=${user?.email}`);
          ...
          <tbody>
            {classes?.map((classs, index) => (
              <tr key={classs._id} className="text-center">
                ...
                <td>
                  <span className="bg-yellow-300 py-2 px-4 font-semibold rounded-md">{classs.status}</span>
                </td>
                <th>
                  <Link to="/dashboard/payment" state={{ price: classs.price, classes: [classs] }}>
                    <button className="btn btn-xs btn-warning">Pay Now</button>
                  </Link>
                  <button className="btn btn-xs btn-error ml-4" onClick={() => handelDeleteClass(classs._id)}>
                    Cancel
                  </button>
                </th>
              </tr>
